...
export default function EditPurchaseOrderPage() {
    ...
    async function loadData() {
      ...
      try {
        ...
        setForm({
          ...
          rim: toNumber(data.rim),
          price: toNumber(data.price),
          delivery_completed_dates: normalizeArray(data.delivery_completed_dates?.map(toDateInput), null),
          partial_billing_quantities: normalizeArray(data.partial_billing_quantities?.map((value) => toNumber(value)), 0),
          total_keping: toNumber(data.total_keping),
  ...
  if (loading || !form) {
    return <div className="p-6 text-sm text-gray-500">Memuat data BKOrder...</div>;
  }
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Ubah data BKOrder sesuai header Excel client terbaru.</p>
      </div>
